...
import radioPlaylist from "@/lib/radio-playlist.json";
...
export function usePlayer(interacted: boolean) {
  ...
  const radioEntriesRef = useRef<DiscoveredVideo[]>(
    radioPlaylist.entries.map((entry) => ({ ...entry, thumbnailUrl: "" })),
  );
  const radioLoadedRef = useRef<string | null>(radioPlaylist.playlistId ?? null);
  ...
  useEffect(() => {
    // Radio mode: the playlist's entries come from the static radio-playlist
    // snapshot (generated by `npm run refresh-radio`, no API key); navigation
    // then just indexes into those entries.
    if (isRadio) {
